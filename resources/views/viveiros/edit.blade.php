            ...
            <form method="POST" action="{{route('viveiros.update', ['viveiro' => $viveiro->id])}}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="flex flex-col">
                        ...
                        <input class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="text" name="name" value="{{$viveiro->name}}" required>
                    </div>

                    <div class="flex flex-col">
                        <label class="text-gray-700 mb-1" for="width">Largura</label>
                        <input class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="number" name="width" value="{{$viveiro->width}}" required>
                    </div>

                    <div class="flex flex-col">
                        <label class="text-gray-700 mb-1" for="length">Comprimento</label>
                        <input class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="number" name="length" value="{{$viveiro->length}}" required>
                    </div>
                </div> 
