<x-app-layout>
    ...
    <h3 class="text-gray-700 text-3xl font-medium mb-6">Controle de Estoque</h3>
    <div class="mt-4 flex justify-center">
        <div class="shadow-lg w-full sm:w-1/2 rounded-lg mt-8">
            <h2 class="text-xl py-3 rounded-t-lg px-6 text-white bg-gray-800 font-bold">Adicionar Entrada</h2>
            <div class="p-6">
                <form action="{{ route('estoque.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="flex flex-col">
                            ...
                            <input type="number" id="quantidade" name="quantidade" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" required>
                            @error('quantidade')
                                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="flex flex-col">
                            ...
                            <input type="date" id="lote" name="lote" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" required>
                            @error('lote')
                                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="flex flex-col">
                            ...
                            <input type="date" id="validade" name="validade" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" required>
                            @error('validade')
                                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
